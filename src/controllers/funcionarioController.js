const { FuncionarioRepository } = require("../models/funcionarioModel");

const funcionarioModel = new FuncionarioRepository();
...
function listar(req, res) {
  funcionarioModel
    ...
    .catch(function (erro) {
      console.log(erro);
      console.log(
        "\n Houve um erro ao carregar funcionários! Erro: ",
        erro.sqlMessage
      );
...
function selecionar(req, res) {
  ...
  if (id == undefined) {
    ...
    funcionarioModel
      .selecionar(id)
      .then(function (pegarDadosFucionario) {
      ...
      .catch(function (erro) {
        console.log(erro);
        console.log(
          "\nHouve um erro ao realizar a captura dos dados! Erro: ",
          erro.sqlMessage
        );
        res.status(500).json(erro.sqlMessage);
      });
      ...
      .then(function (resultadoAutenticar) {
        ...
        if (resultadoAutenticar.length == 1) {
          ...
          res
            .status(500)
            .send(
              "Erro no servidor do aplicativo. Contate seu administrador de TI."
            );
        }
      })
      .catch(function (erro) {
...
function terminarCadastro(req, res) {
  ...
  funcionarioModel
    .terminarCadastro(id, nome, dataNascimeto, senha)
    .then(function (resultado) {
      res.json(resultado);
    })
    .catch(function (erro) {
...
function cadastrar(req, res) {
  ...
  funcionarioModel
    .cadastrar(nome, funcao, dataNasc, email, senha)
    .then(function (resultado) {
      res.json(resultado);
    })
    .catch(function (erro) {
